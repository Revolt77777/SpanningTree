...
class Switch(StpSwitch):
    ...
    def process_message(self, message: Message):
        ...
        # A better root or A better path to root
        if (message.root, message.distance+1, message.origin) < (self.root, self.distToRoot, self.rootPort):
            self.root = message.root
            self.distToRoot = message.distance + 1

            if self.rootPort and self.rootPort in self.activeLinks:
                self.activeLinks.remove(self.rootPort)
            self.activeLinks.add(message.origin)

            self.rootPort = message.origin

        # The origin of the message is relying on Switch to get to Root
        if message.pathThrough and message.origin not in self.activeLinks:
            self.activeLinks.add(message.origin)

        # The origin of the message is not relying on Switch, and it is not the rootPort
        if not message.pathThrough and message.origin != self.rootPort and message.origin in self.activeLinks:
            self.activeLinks.remove(message.origin)

        # Send message if ttl >= 0
        if message.ttl >= 1:
            for target in self.links:
                new_message = Message(self.root, self.distToRoot, self.switchID, target, target == self.rootPort, message.ttl-1)
    ...
    def generate_logstring(self):
        ...
        links = [f"{self.switchID} - {neighbor}" for neighbor in sorted(self.activeLinks)]
        return ', '.join(links)
